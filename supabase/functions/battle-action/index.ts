// Process a single battle action: validate it server-side, mutate state, log
// POST { battleId, action: 'attack'|'skill'|'defend'|'forfeit', skillSlug?, targetSlot? }

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.45.0';
import {
  resolveHit, applyEffect, tickStatusEffects, tickCooldowns, isStunned,
  makeRng, ParticipantState, SkillDef,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const actor = participants.find((p: any) => p.user_id === user.id) as ParticipantState | undefined;
    if (!actor) return j({ error: 'not a participant' }, 403);

    const targetSlotResolved = typeof targetSlot === 'number' ? targetSlot : participants.find((p: any) => p.slot !== actor.slot)!.slot;
    ...
    if (action === 'forfeit') {
      ...
      await admin.from('battle_actions').insert({
        ...
        actor_user_id: user.id, actor_slot: actor.slot,
        action_type: 'forfeit', result: { winner: target.snapshot.user_id },
      });
    ...
    if (isStunned(actor)) {
      ...
      tickCooldowns(actor);
      await commitTurn(admin, battle, participants, actor, target, 'stunned', null);
      return j({ ok: true, stunned: true });
    }
    ...
    // End-of-turn: tick target's status (DoT, expire), tick actor cooldowns
...
async function commitTurn(admin: any, battle: any, participants: any[], actor: any, target: any, action: string, skillSlug: string | null, result: any = {}) {
  // Persist participants
  await admin.from('battle_participants').update({
    hp: actor.hp, energy: actor.energy, rage: actor.rage,
    status_effects: actor.status_effects, cooldowns: actor.cooldowns,
  }).eq('battle_id', battle.id).eq('slot', actor.slot);
  await admin.from('battle_participants').update({
    hp: target.hp, energy: target.energy, rage: target.rage,
    status_effects: target.status_effects, cooldowns: target.cooldowns,
  ...
  if (target.hp <= 0) {
    ...
    // ELO update (simple +/-25)
    if (actor.snapshot.user_id && target.snapshot.user_id) {
      await admin.rpc; // no-op placeholder
      await admin.from('profiles').update({ wins: (await getCount(admin, actor.snapshot.user_id, 'wins')) + 1, elo_rating: (await getElo(admin, actor.snapshot.user_id)) + 25 }).eq('user_id', actor.snapshot.user_id);
      await admin.from('profiles').update({ losses: (await getCount(admin, target.snapshot.user_id, 'losses')) + 1, elo_rating: Math.max(0, (await getElo(admin, target.snapshot.user_id)) - 25) }).eq('user_id', target.snapshot.user_id);
    }
    return;
  }
